...
public class NFA {
    ...
    private LexGraph graph = new LexGraph();
    ...
    public NFA(ArrayList<ArrayList<String>> regExpPostfix, ArrayList<String> finalStates, ArrayList<ArrayList<String>> keywords, ArrayList<ArrayList<String>> punc, LexGraph graph) {
        ...
        System.out.println("NFA: " + graph.getTransitionTable());
    ...
    public void addPriorities() {
        ...
        for (ArrayList<String> arr : keywords) {
            for (String str : arr) {
                priorities.add(str);
            }
        }

        for (ArrayList<String> arr : punc) {
            for (String str : arr) {
                priorities.add(str.replace("\\", ""));
            }
        }
        ...
        for (int i = 0; i < keywords.size(); i++) {
            for (int j = 0; j < keywords.get(i).size(); j++) {
                ...
                for (int k = 0; k < charr.length; k++) {
                    ...
                    String input = String.valueOf(charr[k]);
                    if (!input.equals("\\L")) {
                        input = input.replace("\\", "");
                    }
                    graph.connect(s1, s2, input);
        ...
        for (int i = 0; i < regExpPostfix.size(); i++) {
            for (int j = 0; j < regExpPostfix.get(i).size(); j++) {
                if (regExpPostfix.get(i).get(j).equals("|")) {
                    ...
                    String s2 = String.valueOf(stateNum);
                    graph.createState(s2);
                    String input = regExpPostfix.get(i).get(j);
                    if (!input.equals("\\L")) {
                        input = input.replace("\\", "");
                    }
                    graph.connect(s1, s2, input);
